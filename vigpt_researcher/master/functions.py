...
async def choose_agent(query, cfg):
    ...
    try:
        ...
        return "Default Agent", "Bạn là một trợ lý nghiên cứu trí tuệ nhân tạo với vai trò là một nhà tư duy phê bình. Mục đích duy nhất của bạn là viết các báo cáo được đánh giá cao về nội dung được giao một cách khách quan và có cấu trúc."
...
async def summarize(query, content, agent_role_prompt, cfg, websocket=None):
    ...
    async def handle_task(url, chunk):
        ...
        if summary:
                await stream_output("logs", f"🌐 Đang tóm tắt url: {url}", websocket)
...
async def summarize_url(query, raw_data, agent_role_prompt, cfg):
    ...
    try:
        ...
        print(f"{Fore.RED}Error in summarizes: {e}{Style.RESET_ALL}")
    return summary
