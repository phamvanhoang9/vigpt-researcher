...
class VIGPTResearcher:
    ...
    async def run(self):
        ...
        await stream_output("logs", f"✍️ Đang viết {self.report_type} cho nhiệm vụ tìm kiếm: {self.query}...", self.websocket)
    ...
    async def get_context_by_urls(self, urls):
        ...
        await stream_output("logs",
                            f"🧠 Tôi sẽ tiến hành tìm kiếm dựa trên các link url đã cung cấp: {new_search_urls}...",
                            self.websocket)
    ...
    async def get_new_urls(self, url_set_input):
        ...
        for url in url_set_input:
            if url not in self.visited_urls:
                await stream_output("logs", f"✅ Đang thêm nguồn url để tìm kiếm: {url}\n", self.websocket) 
    ...
    async def get_context_by_search(self, query):
        ...
        await stream_output("logs",
                            f"🧠 Tôi sẽ tiến hành tìm kiếm dựa trên nội dung đã cung cấp: {sub_queries}...",
                            self.websocket)
        
        # Run sub-queries
        for sub_query in sub_queries:
            await stream_output("logs", f"\n🔎 Đang tìm kiếm cho '{sub_query}'...", self.websocket)
    ...
    async def get_similar_content_by_query(self, query, pages):
        await stream_output("logs", f"📃 Đang tìm kiếm thông tin liên quan đến nội dung cần tìm: {query}...", self.websocket)
        # Summarize Raw Data
