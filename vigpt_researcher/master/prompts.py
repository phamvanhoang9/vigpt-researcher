...
def generate_report_prompt(question, context, report_format="apa", total_words=1000):
    ...
    return f'Thông tin: """{context}"""\n\n' \
           f'Sử dụng thông tin trên, hãy trả lời câu hỏi hoặc nhiệm vụ sau: "{question}" trong một báo cáo chi tiết --' \
           " Báo cáo nên tập trung vào việc trả lời câu hỏi, phải được cấu trúc tốt, cung cấp thông tin," \
           ...
           f"Sử dụng một lối viết không thiên vị và cảm giác như một nhà báo.\n" \
...
def generate_outline_report_prompt(question, context, report_format="apa", total_words=1000):
    ...
    return f'"""{context}""" Dựa trên thông tin đã cung cấp, hãy tạo ra một phác thảo cho một báo cáo nghiên cứu bằng cú pháp Markdown' \
       f' cho câu hỏi hoặc chủ đề sau: "{question}". Phác thảo nên cung cấp một khung cảnh cấu trúc tốt' \
       ' cho báo cáo nghiên cứu, bao gồm các phần chính, phần con và các điểm chính cần được bao quát.' \
       ' Báo cáo nghiên cứu nên chi tiết, cung cấp thông tin, sâu sắc và tối thiểu là 1,200 từ.' \
...
def auto_agent_instructions():
    return """
        Nhiệm vụ này liên quan đến việc nghiên cứu một chủ đề cụ thể, bất kể độ phức tạp của nó hay sự có sẵn của một câu trả lời dứt khoát. Nghiên cứu được thực hiện bởi một máy chủ cụ thể, được xác định bởi loại và vai trò của nó, với mỗi máy chủ đòi hỏi các hướng dẫn riêng biệt.
        Đại lý
        ...
        response:
        {
            ...
            "agent_role_prompt": "Bạn là một trợ lý hướng dẫn du lịch trí tuệ nhân tạo đã đi khắp thế giới. Mục tiêu chính của bạn là soạn thảo các báo cáo du lịch hấp dẫn, sâu sắc, không thiên vị và được sắp xếp phương pháp về các địa điểm được cung cấp, bao gồm lịch sử, địa điểm thu hút và những cái nhìn về văn hóa."
        }
    """
...
def generate_summary_prompt(query, data):
    ...
    return f'{data}\n Sử dụng văn bản trên, tóm tắt nó dựa trên nhiệm vụ hoặc câu hỏi sau: "{query}".\n Nếu câu hỏi ' \
           f'không thể được trả lời bằng văn bản, BẠN PHẢI tóm tắt văn bản một cách ngắn gọn.\n Bao gồm tất cả thông tin thực tế ' \
